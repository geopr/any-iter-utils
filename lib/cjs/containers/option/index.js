...
class Option {
    ...
    okOr(val) {
        return this.match({
            Some: (containers_1.Result.Ok),
            None: () => containers_1.Result.Err(__classPrivateFieldGet(this, _Option_instances, "m", _Option_unwrapF).call(this, val)),
        });
    }
    /**
    ...
    iter() {
        const that = this;
        return containers_1.SyncIter.from((0, utils_1.fromGen)(function* () { yield that; }));
    }
    and(val) {
        return this.match({
            Some: (curr) => (0, utils_1.isFunction)(val) ? val(curr) : val,
            None: (None),
        });
    }
    or(val) {
...
_Option_instances = new WeakSet(), _Option_unwrapF = function _Option_unwrapF(val) {
    return (0, utils_1.isFunction)(val) ? val() : val;
};
/**
