 ...
 */
export function filterMap<T, R>(iterable: Iterable<T>, cb: (value: T) => Option<R>): IterableIterator<R>;
 ...
 */
export function filterMap<T, R>(iterable: AsyncIterable<T>, cb: (value: T) => Option<R>): AsyncIterableIterator<R>;

/**
 * service overload
 */
export function filterMap<T, R>(iterable: AnyIterable<T>, cb: (value: T) => Option<R>): AnyIterableIterator<R>;

export function filterMap<T, R>(iterable: AnyIterable<T>, cb: (value: T) => Option<R>): AnyIterableIterator<R> {
  const 
    options = map(iterable, cb),
    onlySomes = filter(options, (opt) => opt.isSome());
