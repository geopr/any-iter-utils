 ...
 */
export function intoIter<T>(iterable: Iterable<T>): SyncIter<T>;
 ...
 */
export function intoIter<T>(iterable: AsyncIterable<T>): AsyncIter<T>;

/**
 * service overload
 */
export function intoIter<T>(iterable: AnyIterable<T>): SyncIter<T> | AsyncIter<T>;
 ...
 */
export function intoIter(from: number, to?: number): SyncIter<number>;
 ...
 */
export function intoIter<T extends Record<PropertyKey, any>>(obj: T): SyncIter<Entry<keyof T, T[keyof T]>>;

export function intoIter<T>(
  value: AnyIterable<T> | Record<PropertyKey, any> | number,
  to?: number,
): SyncIter<any> | AsyncIter<any> {
  if (isNumber(value)) {
